...
description: Turn mixed personal daily logs, journals, chat fragments, reminders, and informal "流水账" notes into a complete Markdown reflection summary for personal growth and knowledge storage, then optionally publish the confirmed entry to the user's configured GitHub repository. Use when the user says phrases like "生成一下今天流水账", "总结今天流水账", "做一下今日复盘", asks to summarize today's personal record, create an入库 Markdown,整理个人流水账, analyze emotional state/work/interpersonal relationships, extract facts/thoughts/preferences/highlights/problems/actions, or convert raw life notes into a coherent daily archive.
---
...
2. Show the raw record first. Present the source notes that will be summarized under a short "原始记录" heading. Lightly normalize obvious formatting only; do not rewrite the evidence at this stage.
3. Read the source notes as raw evidence. Accept mixed formats: prose, bullets, chat logs, task lists, screenshots transcribed by the user, or pasted fragments.
4. Group entries by topic before summarizing. Common topics include work, projects, relationships, health, money, learning, travel, tools, household, decisions, and emotions.
5. Separate evidence, reflection, and durable information:
...
6. Mark uncertainty explicitly. Use "待确认" for guesses, weak patterns, or inferred preferences.
7. Filter noise internally. Do not show a "不应入库的噪音" section unless the user explicitly asks to see what was omitted.
8. Produce one coherent Markdown document with a personal-growth orientation. Do not output a fragmented extraction unless the user asks for raw tables.
9. Ask the user to confirm whether anything needs to be modified. Do not publish or push anything before explicit confirmation.
10. If the user says no changes are needed or explicitly approves publishing, save the confirmed Markdown entry to a temporary Markdown file, then run `scripts/publish_daily_reflection.py` to commit and push it to the user's configured GitHub repository. Follow `references/github-publish-workflow.md`.
11. After a successful publish, delete the temporary Markdown input file. Keep it only if publishing fails, so the user can inspect or retry from that local file.

## Direct Invocation

When the user says "生成一下今天流水账", "总结今天流水账", "做一下今日复盘", or a similar short request, immediately produce the daily Markdown reflection for the current date using available conversation context and any notes the user provides in the same turn. Do not ask the user to choose a structure; use the default output structure.
...
- Preserve the source language locally. If the user wrote a fact, phrase, preference, or project context in Chinese, summarize that item in Chinese; if the user wrote it in English, keep it in English. For mixed Chinese-English notes, keep the natural mixed style instead of translating everything into one language.
- Use Chinese section headings by default, but do not translate embedded English terms, quotes, tool names, task names, or user wording unless the user explicitly asks for translation.
